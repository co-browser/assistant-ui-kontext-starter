...
import { KontextConnectButton, KontextStatus } from '@kontext.dev/kontext-sdk/components';
import { Button } from '@/components/ui/button';
import { CheckCircle, Loader2 } from 'lucide-react';
import { useKontextCookie } from '@/hooks/useKontextCookie';

export function KontextConnectionStatus() {
  const { isConnected, connectGmail, disconnect, isLoading, error, userId } = useKontext();
  ...
  if (isConnected) {
    return (
      <div className="flex flex-col gap-2 px-3 py-2">
        ...
        {userId && (
          <span className="text-xs text-muted-foreground">User ID: {userId}</span>
        )}
      </div>
    );
  }
